...
# We initialize them to some random values based on teh normal distribution.
tf_size_factor = tf.Variable(np.random.randn(), name="size_factor")
tf_price_offset = tf.Variable(np.random.randn(), name="price_offset")

# 2. Define the operations for the predictiong values
...
# Launch the graph int he session
with tf.Session() as sess:
    sess.run(init)

    # set how often to display training process and number of training iterations
    display_every = 2
